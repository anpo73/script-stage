import fs from 'fs'

export interface ParsedTestCase {
  id: string // TC-001
  title: string // User login with valid credentials
  stepTitles: string[]
}

export interface ParsedTest {
  suiteTitle: string
  testCases: ParsedTestCase[]
}

/**
 * Parse TypeScript test file and extract test suite with test cases
 *
 * IDs are extracted "as is" from brackets without any modifications:
 * - test('[TC01-01] Title') → ID: 'TC01-01'
 * - test('[TC01-01-MANUAL] Title') → ID: 'TC01-01-MANUAL' (kept as is)
 * - test('[USER-LOGIN-E2E] Title') → ID: 'USER-LOGIN-E2E' (kept as is)
 * - test('Title') → ID: '' (no brackets, cannot determine ID)
 * - test('TC01-01: Title') → ID: '' (not in brackets format)
 *
 * Validator will handle suffix matching by comparing against MD ID as reference
 */
export function parseTestFile(filePath: string): ParsedTest {
  const content = fs.readFileSync(filePath, 'utf-8')

  // 1. Extract suite title from test.describe('Title', ...)
  const describeMatch = content.match(/test\.describe\s*\(\s*['"`]([^'"`]+)['"`]/)
  if (!describeMatch) {
    throw new Error(`No test.describe() found in ${filePath}`)
  }

  const suiteTitle = describeMatch[1]
  const testCases: ParsedTestCase[] = []

  // 2. Split content by test( to find test cases
  // Supports: test(), test.skip(), test.only(), test.fixme()
  // Excludes: test.describe() (negative lookahead)
  const testSplits = content.split(/\n\s*test(?!\.describe)(?:\.\w+)?\s*\(/)

  for (let i = 1; i < testSplits.length; i++) {
    const testBlock = testSplits[i]

    // Extract test case ID and title
    // Pattern with ID in brackets: '[ANY-ID] Title'
    // Pattern without brackets: 'Title' (cannot determine ID, use only title)

    // Determine which quote type was used
    const quoteMatch = testBlock.match(/^\s*(['"`])/)
    if (!quoteMatch) continue

    const quote = quoteMatch[1]

    // Find matching closing quote manually to handle quotes inside the string
    const startPos = quoteMatch.index! + quoteMatch[0].length - 1
    let endPos = startPos + 1
    let escaped = false

    while (endPos < testBlock.length) {
      const char = testBlock[endPos]

      if (escaped) {
        escaped = false
        endPos++
        continue
      }

      if (char === '\\') {
        escaped = true
        endPos++
        continue
      }

      if (char === quote) {
        // Found matching closing quote
        break
      }

      endPos++
    }

    let fullTitle = testBlock.slice(startPos + 1, endPos)

    // Unescape common escape sequences
    fullTitle = fullTitle
      .replace(/\\'/g, "'")
      .replace(/\\"/g, '"')
      .replace(/\\`/g, '`')
      .replace(/\\\\/g, '\\')
      .replace(/\\n/g, '\n')
      .replace(/\\r/g, '\r')
      .replace(/\\t/g, '\t')

    // Extract ID and title
    const withIdMatch = fullTitle.match(/^\[([^\]]+)\]\s*(.+)$/)
    let tcId = ''
    let tcTitle = ''

    if (withIdMatch) {
      // ID in brackets format
      tcId = withIdMatch[1].trim()
      tcTitle = withIdMatch[2].trim()
    } else {
      // No brackets - entire string is title
      tcId = ''
      tcTitle = fullTitle.trim()
    }

    // Extract steps from this test block
    // Find all step calls and extract their titles (handle quotes inside titles)
    const stepPattern = /(?:manualStep|test\.step|playwrightTest\.step)\s*\(\s*(['"`])/g
    const stepTitles: string[] = []

    let match
    while ((match = stepPattern.exec(testBlock)) !== null) {
      const startPos = match.index + match[0].length - 1 // Position of opening quote
      const quote = match[1]

      // Find matching closing quote
      let endPos = startPos + 1
      let escaped = false

      while (endPos < testBlock.length) {
        const char = testBlock[endPos]

        if (escaped) {
          escaped = false
          endPos++
          continue
        }

        if (char === '\\') {
          escaped = true
          endPos++
          continue
        }

        if (char === quote) {
          // Found matching closing quote
          let stepTitle = testBlock.slice(startPos + 1, endPos)

          // Unescape common escape sequences
          stepTitle = stepTitle
            .replace(/\\'/g, "'")
            .replace(/\\"/g, '"')
            .replace(/\\`/g, '`')
            .replace(/\\\\/g, '\\')
            .replace(/\\n/g, '\n')
            .replace(/\\r/g, '\r')
            .replace(/\\t/g, '\t')

          stepTitles.push(stepTitle.trim())
          break
        }

        endPos++
      }
    }

    testCases.push({
      id: tcId,
      title: tcTitle,
      stepTitles
    })
  }

  if (testCases.length === 0) {
    throw new Error(`No test cases found in ${filePath}`)
  }

  // Validate no duplicate IDs (only check IDs in brackets format)
  const idsWithBrackets = testCases.filter(tc => tc.id !== '').map(tc => tc.id)
  const duplicateIds = idsWithBrackets.filter((id, index) => idsWithBrackets.indexOf(id) !== index)

  if (duplicateIds.length > 0) {
    const uniqueDuplicates = [...new Set(duplicateIds)]
    throw new Error(
      `Duplicate test case IDs found in ${filePath}:\n` +
      uniqueDuplicates.map(id => `  - [${id}]`).join('\n') +
      `\n\nEach test() must have a unique ID within the suite.`
    )
  }

  return { suiteTitle, testCases }
}
