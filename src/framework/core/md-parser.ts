...
export function parseMDFile(fileName: string): ParsedMD {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('# ') && !suiteTtl) {
      const suiteWithIdMatch = trimmed.match(/^#\s*\[([^\]]*)\]\s*(.+)/)
      const suiteNoIdMatch = trimmed.match(/^#\s+(.+)/)
      ...
      throw new Error(`Invalid test suite title format in ${mdPath}: ${trimmed}`)
    }
  ...
  if (!suiteTtl) {
    throw new Error(`No test suite title found in ${mdPath}`)
  }
...
function findMdFile(fileName: string): string | null {
  ...
  const result = search(PATHS.TEST_SUITES)
